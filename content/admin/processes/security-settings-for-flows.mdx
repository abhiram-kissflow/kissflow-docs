...
Global permissions are the default access rule that applies to every flow in your account. Today, **Lookup** is the only artifact with a true global toggle — an **All flows** setting that's turned on by default, so any flow in your account can look up this flow's data unless you turn it off or override it for a specific flow.
...
1. On the **Security** page, add the flow you want to configure under **Customized permissions**.
2. Choose the permissions to grant — for **Lookup**, select **All fields** to expose every field, or **Select fields** to expose only specific ones.
3. Save your changes.
